      ...
      if (HAL_GetTick() - last_update > 500) {
          ...
          if (RC522_CheckCard(card_uid) == 0) {
              ...
              if (HAL_GetTick() - last_card_detect_time > 2000) {
                  is_card_detected = 0;
                  card_uid[0] = 0; card_uid[1] = 0; card_uid[2] = 0; card_uid[3] = 0;
              }
          }
...
static void MX_RTC_Init(void)
{
  ...
  */
  ...
  {
    // Error_Handler(); // Commented out to prevent freeze
  }
...
int32_t HX711_Read(void) {
    ...
    while(HAL_GPIO_ReadPin(GPIOE, GPIO_PIN_2) == GPIO_PIN_SET) {
        if (HAL_GetTick() - start_tick > 150) return 0; // Tránh treo chip khi đứt dây cảm biến
    }
    ...
    for (int i = 0; i < 24; i++) {
        ...
        for (volatile int d = 0; d < 100; d++) {} // Tốc độ STM32F429 180MHz quá nhanh! Phải tạo trễ ~1us (> 200ns)
        ...
        for (volatile int d = 0; d < 100; d++) {} // Trễ 1us cho xung cạnh xuống
    }
...
uint8_t RC522_CheckCard(uint8_t *id)
{
    ...
    for (int i=0; i<16; i++) buf[i] = 0x00; // Xóa sạch buffer cũ
    buf[0] = 0x93;
    buf[1] = 0x20;
